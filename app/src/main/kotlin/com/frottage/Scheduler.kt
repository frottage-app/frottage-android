    ...
    override suspend fun doWork(): Result =
            withContext(Dispatchers.IO) {
                try {
                    ...
                    logToFile(
                            applicationContext,
                            "[WorkerDebug] Worker started. Current UTC time (now): $now"
                    )
                    ...
                    logToFile(
                            applicationContext,
                            "[WorkerDebug] Using schedule type: ${schedule::class.simpleName}"
                    )

                    val previousUpdateTime = schedule.prevUpdateTime(now)
                    logToFile(
                            applicationContext,
                            "[WorkerDebug] Calculated prevUpdateTime(now): $previousUpdateTime"
                    )

                    val activeTimestampKey =
                            schedule.getActivePeriodTimestampKey(
                                    now
                            ) // This internally calls prevUpdateTime(now) again
                    logToFile(
                            applicationContext,
                    ...
                    try {
                        ...
                        val imageDetailsResult =
                                if (SettingsManager.currentWallpaperSource
                                                .supportsFrottageRatingSystem
                                ) {
                                    ImageMetadataService.fetchImageDetails(
                                            applicationContext,
                                            activeTimestampKey,
                                            targetKey,
                                    )
                        ...
                        val imageId = imageDetailsResult?.imageId
                        ...
                        val jsonProperties = buildJsonObject {
                            ...
                            put(
                                    "theme",
                                    JsonPrimitive(
                                            if (FrottageApiService.isDarkTheme(applicationContext))
                                                    "dark"
                                            else "light"
                                    )
                            )
                            put("source", JsonPrimitive("schedule"))
                            imageId?.let { actualImageId ->
                                put("image_id", JsonPrimitive(actualImageId))
                            }
                            // Add battery optimization status
                            val batteryStatusNullable =
                                    BatteryUtils.getBatteryOptimizationExemptionStatusForAnalytics(
                                            applicationContext
                                    )
